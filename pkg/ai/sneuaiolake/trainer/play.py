...
def create_ai(model_path):
    """AIを作成"""
    if model_path:
        model = load_model(model_path)
        return NeuralNetworkAI(model, f"NeuralNetworkAI ({model_path})")
    else:
        return TrivialAI()

def play(p0_model_path, p1_model_path, output_dir, num_games, prefix, show_log):
    """自己対戦を実行"""
    # AIの作成
    p0_ai = create_ai(p0_model_path)
    p1_ai = create_ai(p1_model_path)
...
def main():
    ...
    parser.add_argument('--p0-model', type=str, default=None, help='P0として使用するKerasモデルのパス（.keras形式）')
    parser.add_argument('--p1-model', type=str, default=None, help='P1として使用するKerasモデルのパス（.keras形式）')
    ...
    # 自己対戦の実行
    play(args.p0_model, args.p1_model, args.output, args.games, args.prefix, args.show_log)
